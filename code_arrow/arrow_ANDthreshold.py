...
from utils.covest import NetBanding, ANDNetBanding
...
from my_api import *
# %%
# arrow, LSRthreshold, use_correlation = False
repetition = 100
cv_option = 'brute'
cmap = 'gist_gray_r'
num_cv = 50
folder = 'data_arrow'
simu_str = 'ANDthreshold'
cov_str = 'arrow'
            ...
            for prob in [1]: # [0.9, 0.99, 1]:
                for qrob in [0]: # [0, 0.01, 0.1]:
                    for ord in ['fro', 2]:
                        ...
                        for is_threshold in [True, False]:
                            print(ord, tau, prob, qrob, is_threshold)
                            err_cor = []
                            err_cov = []   
                            ...
                            for i in range(repetition): 
                                ...
                                m = ANDNetBanding(X, G = hatL, use_correlation = False, num_cv = num_cv)
                                if is_threshold:
                                    param_threshold = m.params_by_cv(cv_option = cv_option)
                                    S_est = m.fit(param_threshold)
                                else:
                                    S_est = m.sample_cov() * hatL
                                R_est = cov2cor(S_est)

                                assert all([(x == y) or x < 5 or y < 5 for x, y in zip(*np.where(S_est > 0))])
                            ...
                            simu_params = [tau, prob, qrob, cv_option, num_cv, is_threshold]
